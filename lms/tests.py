...
class LMSPermissionsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.moderator_group, created = Group.objects.get_or_create(
            name=MODERATOR_GROUP_NAME
        )
        self.moderator_user = User.objects.create(
            email="moderator@lms.com", is_staff=True, phone="3"
        )
...
class CourseTestCase(LMSPermissionsTestCase):
    ...
    def test_course_list_retrieve_access(self):
        ...
        self.assertEqual(
            self.client.get(self.course_list_url).status_code, status.HTTP_200_OK
        )
        self.assertEqual(
            self.client.get(self.course_detail_url).status_code, status.HTTP_200_OK
        )

        self.unauthenticate()
        self.assertEqual(
            self.client.get(self.course_list_url).status_code,
    ...
    def test_course_update_by_moderator_allowed(self):
        ...
        response = self.client.patch(
            self.course_detail_url, {"title": "Moderator Updated"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_course_update_by_non_owner_forbidden(self):
        ...
        self.authenticate_as_user(self.non_owner_user)
        response = self.client.patch(
            self.course_detail_url, {"title": "Non-Owner Attempt"}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class LessonTestCase(LMSPermissionsTestCase):
    ...
    def test_lesson_create_retrieve_update_delete_by_owner(self):
        ...
        # 3. UPDATE
        response = self.client.patch(
            reverse("lms:lesson-detail", args=[new_lesson_pk]), {"title": "Updated"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # 4. DELETE
        response = self.client.delete(
            reverse("lms:lesson-detail", args=[new_lesson_pk])
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_lesson_update_by_moderator_allowed(self):
        ...
        self.authenticate_as_user(self.moderator_user)
        response = self.client.patch(
            self.lesson_detail_url, {"title": "Mod Updated Lesson"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
