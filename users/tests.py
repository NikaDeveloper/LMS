...
class UserAuthTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.owner_profile_url = reverse('users:user-profile', args=[self.owner_user.pk])
        self.other_profile_url = reverse('users:user-profile', args=[self.other_user.pk])
    ...
    def test_profile_retrieve_public_restricted_data(self):
        ...
        response = self.client.get(self.owner_profile_url)  # other_user смотрит owner_user
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_profile_update_by_owner_allowed(self):
        ...
        self.client.force_authenticate(user=self.owner_user)
        response = self.client.patch(self.owner_profile_url, {"first_name": "UpdatedOwner"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_profile_update_by_non_owner_forbidden(self):
        """ Другой пользователь не может обновить чужой профиль (403) """
        self.client.force_authenticate(user=self.other_user)
        response = self.client.patch(self.owner_profile_url, {"first_name": "Hack Attempt"})
...
class PaymentTestCase(APITestCase):
    ...
    def test_payment_list_filtering(self):
        ...
        Payment.objects.create(user=self.user, course=self.course, amount=1000, payment_method='cash')
